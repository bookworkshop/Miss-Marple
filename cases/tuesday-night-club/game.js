...
function renderVerdictScene(message = '') {
  ...
  if (!crimeSequenceSolved) {
    ...
    content = `<section class="crime-part sequence-part"><header><span>PART 1</span><div><h3>HOW WAS IT DONE?</h3><p>Place the two known opening actions first. The final three events may be arranged in any order.</p></div></header><div class="crime-event-bank">${remaining.map(event => `<button type="button" data-crime-event="${event.id}">${event.text}</button>`).join('') || '<p>All events have been placed.</p>'}</div><ol class="crime-sequence">${[0,1,2,3,4].map(index => `<li>${crimeSequence[index] ? `<button type="button" data-sequence-index="${index}">${crimeEvents.find(event => event.id === crimeSequence[index]).text}</button>` : '<span>Choose the next event</span>'}</li>`).join('')}</ol><button class="crime-action" id="checkCrimeSequence" type="button" ${crimeSequence.length === 5 ? '' : 'disabled'}>CHECK THE SEQUENCE</button></section>`;
  } else if (!gladysMotiveSolved) {
    content = `<section class="crime-part gladys-part"><header><span>PART 2</span><div><h3>ONE PERSON IS STILL MISSING FROM THE PLAN</h3></div></header><article class="gladys-file"><img src="images/gladys-linch.png" alt="Gladys Linch"><div><p class="crime-label">GLADYS LINCH</p><h3>Why did Gladys help Mr Jones?</h3><div class="crime-options"><button type="button" data-gladys-motive="a"><span>A.</span>He promised to pay her a large amount of money.</button><button type="button" data-gladys-motive="b"><span>B.</span>She believed he would marry her after Mrs Jones’s death.</button><button type="button" data-gladys-motive="c"><span>C.</span>She wanted revenge on Miss Clark.</button><button type="button" data-gladys-motive="d"><span>D.</span>She did not know that the decorations were poisoned.</button></div></div></article></section>`;
  } else if (!finalCaseSolved) {
    const complete = Object.values(finalCaseSolution).every(Boolean);
    content = `<section class="crime-part solution-part"><header><span>PART 3</span><div><h3>SOLVE THE CASE</h3><p>Complete the official solution.</p></div></header><article class="official-solution"><div class="solution-file-head"><span>ST. MARY MEAD · OFFICIAL CASE REPORT</span><b>01 / 1932</b></div>${Object.entries(finalSolutionOptions).map(([field,options]) => `<section class="solution-row"><strong>${field.toUpperCase()}:</strong><div class="paper-choice-list">${options.map(option => `<button type="button" class="${finalCaseSolution[field] === option ? 'selected' : ''}" data-solution-field="${field}" data-solution-value="${option}">${option}</button>`).join('')}</div></section>`).join('')}<button class="crime-action" id="checkFinalSolution" type="button" ${complete ? '' : 'disabled'}>CHECK THE SOLUTION</button></article></section>`;
  } else {
...
function bindVerdictInteractions() {
  ...
  document.querySelector('#checkCrimeSequence')?.addEventListener('click', () => {
    const openingIsCorrect = crimeSequence[0] === 'poison' && crimeSequence[1] === 'gladys';
    const finalEventsArePresent = ['remove', 'banting', 'eats'].every(id => crimeSequence.slice(2).includes(id));
    if (!openingIsCorrect || !finalEventsArePresent) { recordMistake(); renderVerdictScene('The first two actions do not fit the plan. The final three events may be placed in any order.'); return; }
